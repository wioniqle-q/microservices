...
pub extern "C" fn encrypt(input: *const u8, input_len: size_t, key: *const u8, encrypted_data_len: *mut size_t) -> *mut u8 {
    ...
    match encrypt_string(str::from_utf8(input_slice).unwrap_or(""), &new_key) {
        Ok(encrypted_data) => {
            ...
            let layout = std::alloc::Layout::from_size_align(encrypted_data_len_value, 1).unwrap();
            let encrypted_data_ptr = unsafe { std::alloc::alloc(layout) as *mut u8 };
            if !encrypted_data_ptr.is_null() {
                ...
                unsafe { *encrypted_data_len = encrypted_data_len_value as size_t };

                unsafe { std::ptr::write_bytes(input as *mut u8, 0, input_len) };
...
pub extern "C" fn decrypt(encrypted_data: *const u8, encrypted_data_len: size_t, key: *const u8, decrypted_data_len: *mut size_t) -> *mut u8 {
    ...
    match decrypt_string(encrypted_data_slice, &new_key) {
        Ok(decrypted_data) => {
            ...
            let layout = std::alloc::Layout::from_size_align(decrypted_data_len_value, 1).unwrap();
            let decrypted_data_ptr = unsafe { std::alloc::alloc(layout) as *mut u8 };
            if !decrypted_data_ptr.is_null() {
                ...
                unsafe { *decrypted_data_len = decrypted_data_len_value as size_t };

                unsafe { std::ptr::write_bytes(encrypted_data as *mut u8, 0, encrypted_data_len) };
